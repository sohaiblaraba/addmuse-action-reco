...
if __name__ == '__main__':
    ...
    capture = './data/WIN_20220414_10_24_22_Pro.mp4'
    ...
    while ret:
        ...
        if not pause:
            ...
            for person in people.people:
                if person.active:
                    ...
                    cv2.rectangle(frame, (int(x1), int(y1)), (int(x2), int(y2)), colors[person.id], thickness=1)
                    cv2.putText(frame, 'ID_' + str(person.id), (int(x1), int(y1)), cv2.FONT_HERSHEY_SIMPLEX, 1, colors[person.id], 1, cv2.LINE_AA)
                    ...
                    if abs(person.position[0] - xtargets[0]) <= 100:
                        cv2.circle(frame, (xtargets[0], ytarget), 100, colors[person.id], -1)
                        ...
                        cv2.circle(frame, (xtargets[1], ytarget), 100, colors[person.id], -1)
                        # Position of the text to be displayed inside the circle
                        textsize = cv2.getTextSize(classes[person.get_feature()], cv2.FONT_HERSHEY_SIMPLEX, 0.5, 1)[0]
                    ...
                    for seg in kapao.data['segments'].values():
                        ...
                        cv2.line(frame, pt1, pt2, colors[person.id], 3)
        ...
        if k == ord('s'):
            ...
            com.list_of_people_to_analyze = [0]
        if k == ord(' '):
            pause = not pause
        if k == ord('c'):
            colors = []
